...
│   ├── java/
    ...
    │   ├── karate-config.js
    └── resources/
        └── karate-test.feature <-- UBICACIÓN ESTÁNDAR PARA TODOS LOS FEATURES
```

> **IMPORTANTE**: 
> 1. Todos los escenarios  DEBEN crearse en el archivo `karate-test.feature` ubicado en la ruta `src/test/resources/`.
...
- **La entrada es una colección Postman:** Se usarán sus requests y ejemplos de respuesta para generar los JSON necesarios en `src/test/resources/data` y construir los escenarios para cada caso de uso.
...
- Agrupa features por proyecto en `src/test/resources`.
...
- **Ubicación obligatoria**: `src/test/resources/`
- **Ejemplo**: `src/test/resources/karate-test.feature`
...
4. **Verificación de features**:
   - Confirmar que existe la estructura para features en `src/test/resources/`
   - Crear las carpetas necesarias para los microservicios que se van a utilizar
